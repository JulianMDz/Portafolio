...
const Grainient: React.FC<GrainientProps> = ({
  ...
  useEffect(() => {
    ...
    // The just-revealed theme background (display:none -> block) mounted
    // while hidden, so its first setSize() measured a 0x0 container.
    // Re-measure once it actually has layout dimensions.
    window.addEventListener('themechange', setSize);
    ...
    return () => {
      ...
      window.removeEventListener('themechange', setSize);
